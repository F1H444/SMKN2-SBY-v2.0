// ============================================
// app/(home)/stateSlider.tsx - OPTIMIZED VERSION
// ============================================

// PERUBAHAN YANG DILAKUKAN:
// 1. [OPTIMASI BUNDLE - PERBAIKAN] Impor `lucide-react` (`ChevronLeft`, `ChevronRight`) dikembalikan ke impor standar. Optimasi (tree-shaking) akan ditangani oleh Next.js melalui `next.config.js`.
// 2. [OPTIMASI IMAGE] Menghapus `priority` dan `loading="eager"` dari `next/image` di dalam `Slide`. Karena seluruh komponen `StateSection` sekarang di-lazy load (dari `page.tsx`), gambar di dalamnya tidak boleh menjadi prioritas LCP.
// 3. [OPTIMASI REACT] Komponen `CountUp` sudah menggunakan `IntersectionObserver`, yang sangat baik untuk performa. Semua `memo` dan `useCallback` sudah diimplementasikan dengan benar.

// FULL OPTIMIZED CODE:
// app/(home)/stateSlider.tsx
  ...
  ({
    ...
    >
      <Image
        ...
        // [OPTIMASI IMAGE] Menghapus priority dan loading="eager"
        // priority={index === 0}
...
// PENJELASAN DETAIL:
// - **Image `priority` Removal:** `priority` HANYA boleh digunakan untuk LCP (Largest Contentful Paint) halaman. Karena komponen ini sekarang di-lazy load, gambar di dalamnya tidak akan pernah menjadi LCP. Menghapusnya akan mencegah browser memprioritaskan download gambar yang salah.
// - **Lucide Icon Import:** Sama seperti di `others.tsx`, impor path spesifik mengurangi ukuran bundle.
